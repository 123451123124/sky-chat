...
export function Sidebar({ currentSessionId, onSelectSession, onNewSession, onDeleteSession, user, onLogout }: SidebarProps) {
  ...
  return (
    <aside className="w-72 flex flex-col h-full bg-gray-950 text-gray-300">
      {/* New Chat Button */}
      <div className="p-3">
        <button
          onClick={onNewSession}
          className="w-full px-3 py-2.5 bg-gray-800 hover:bg-gray-700 text-gray-200 rounded-lg transition-colors text-sm flex items-center gap-2 border border-gray-700/50"
        >
      ...
      <div className="flex-1 overflow-y-auto px-2">
        {sessions.length === 0 && (
          <div className="p-3 text-center text-gray-500 text-xs">暂无会话</div>
        )}
        {sessions.map((session) => (
          ...
          >
            <div className="flex-1 min-w-0 flex items-center gap-2">
              <svg className="w-4 h-4 flex-shrink-0 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            <button
              ...
              className="opacity-0 group-hover:opacity-100 p-1 text-gray-500 hover:text-red-400 transition-all rounded hover:bg-gray-700/50 flex-shrink-0"
              title="删除会话"
            >
