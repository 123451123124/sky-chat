...
export function ChatContainer({ sessionId, isNewSession, onSessionCreated }: ChatContainerProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-900 relative">
      {shareToast && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 px-4 py-2 bg-gray-900 dark:bg-gray-700 text-white text-sm rounded-lg shadow-lg animate-fade-in-up">
          {shareToast}
        </div>
      )}
      ...
      {status !== 'idle' && (
        <div className="absolute top-14 left-1/2 -translate-x-1/2 z-10">
          <button
            onClick={handleStop}
            className="px-4 py-2 text-sm text-red-600 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full shadow-sm hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors flex items-center gap-2"
          >
